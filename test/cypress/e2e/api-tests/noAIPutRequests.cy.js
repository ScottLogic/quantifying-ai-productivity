describe("Post requests", () => {
  before(() => {
    cy.request(
      "PUT",
      "/todo/incompleted/fd5ff9df-f194-4c6e-966a-71b38f95e14f"
    ).then((response) => {
  ...
  it("Put: Complete Task 2", () => {
    ...
    cy.request(
      "PUT",
      "/todo/completed/fd5ff9df-f194-4c6e-966a-71b38f95e14f"
    ).then((response) => {
      expect(response.body.success).to.be.true;
      expect(response.body)
        .property("message")
        .to.eq("This task has now been completed.");
    });
    cy.request({
      method: "GET",
      url: "/todo",
      ...
      cy.fixture("staticData").then((staticData) => {
        const creationTime = new Date(response.body[0].completed).getTime();
  ...
  it("Put: Complete Task 2 (Already Completed)", () => {
    ...
    cy.request(
      ...
      expect(response.body)
        .property("message")
        .to.eq("Task already marked complete.");
    });
    cy.request({
      method: "GET",
      url: "/todo",
  ...
  it("Put: Task Not Found", () => {
    cy.request(
      "PUT",
      "/todo/completed/00000000-0000-0000-0000-000000000000"
    ).then((response) => {
      expect(response.status).to.eq(200);
      expect(response.body.success).to.be.false;
