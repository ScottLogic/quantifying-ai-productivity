...
describe("POST Add Task Missing Name", () => {

    it("should add Task with no name and return a response of 400", () => {
        cy.request({
          method: "POST", 
          url: "http://localhost:8080/todo/addTask?name=&description=Task4Description",  
          ...
          if (response.body.path) {
            expect(response.body.path).to.eq("/todo/addTask?name=&description=Task4Description");
          }
        });
    });
    });
